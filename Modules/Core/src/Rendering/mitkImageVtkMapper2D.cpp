...
static void CreateImage( vtkImageData * image)
{
  ...
  image->SetDimensions(4096,1,1);
  image->AllocateScalars(VTK_UNSIGNED_CHAR,4);

  for( int region = 0; region<16;region++)
  {
    double r,g,b;
    vtkMath::HSVToRGB(region/16.0,1.0,1.0,&r,&g,&b);
    for (int x = 0; x < 256; x++)
    {
    unsigned char* pixel =
        static_cast<unsigned char*>(image->GetScalarPointer(region*256+x,0,0));
      pixel[0] = r * 255;
      pixel[1] = g * 255;
      pixel[2] = b * 255;
      pixel[3] = region;
...
void mitk::ImageVtkMapper2D::GenerateDataForRenderer( mitk::BaseRenderer *renderer )
{
  ...
  { //Connect the mapper with the input texture. This is the standard case.
    ...
    localStorage->m_Mapper->SetFragmentShaderCode(
      ...
      "float levelwindow(float x) { return x/1000.0; }\n"
      "float sampleGray(vec2 pos) { return texture2D(texture1, pos).r; }\n"
      "vec4 sampleLookup(float x) { return texture2D(texture2, vec2(x,0.5)).rgba; }\n"
      "void main () {\n"
      "  vec2 deltaA = dFdx(tcoordVCVSOutput.xy); \n"
      "  vec2 deltaB = dFdy(tcoordVCVSOutput.xy); \n"
      "  vec2 v_texcoords = tcoordVCVSOutput.xy; \n"

      "  vec4 cW = sampleLookup(levelwindow(sampleGray(v_texcoords.xy+deltaA)));\n"
      "  vec4 cN = sampleLookup(levelwindow(sampleGray(v_texcoords.xy+deltaB)));\n"
      "  vec4 cO = sampleLookup(levelwindow(sampleGray(v_texcoords.xy-deltaA)));\n"
      "  vec4 cS = sampleLookup(levelwindow(sampleGray(v_texcoords.xy-deltaB)));\n"

      "if( cN.a == cO.a && cO.a == cS.a && cS.a == cW.a ) {"
      "  if( cN.a > 0.0 ) \n"
      "  { gl_FragData[0] = vec4(cN.r,cN.g,cN.b, 1.0); }\n"
      "  else \n"
    ...
    MITK_INFO << "FragmentShader:\n" << localStorage->m_Mapper->GetFragmentShaderCode();


    if(1) // shaderproperty set to rtmode
    {
      localStorage->m_castFilter->SetInputData(localStorage->m_ReslicedImage);
      localStorage->m_castFilter->SetOutputScalarTypeToFloat();
      localStorage->m_castFilter->Update();
      localStorage->m_Texture->SetInputConnection(localStorage->m_castFilter->GetOutputPort());


      vtkSmartPointer<vtkImageData> img = vtkSmartPointer<vtkImageData>::New();
      CreateImage(img);
      localStorage->m_LutTexture->SetInputData(img);

      localStorage->m_Actor->GetProperty()->SetTexture( vtkProperty::VTK_TEXTURE_UNIT_1, localStorage->m_LutTexture);
    ...
    localStorage->m_Actor->GetProperty()->SetTexture( vtkProperty::VTK_TEXTURE_UNIT_0, localStorage->m_Texture);
    contourShadowActor->SetVisibility( false );
  }

  // We have been modified => save this for next Update()
  localStorage->m_LastUpdateTime.Modified();
  //MITK_INFO << "is this mapper used for my rt dose image ?";
}
  ...
  : m_VectorComponentExtractor(vtkSmartPointer<vtkImageExtractComponents>::New())
{
  ...
  m_LutTexture = vtkSmartPointer<vtkNeverTranslucentTexture>::New().GetPointer();
  m_castFilter = vtkSmartPointer<vtkImageCast>::New();
  ...
  m_Mapper = vtkSmartPointer<MITKIMAGEVTKMAPPER2D_POLYDATAMAPPER>::New();
